...
const TOUR_BASE = [
  ...
  { route: '/dashboard', sel: '[data-tour="dash-kpis"]', title: 'Your KPIs', text: 'These tiles summarise incidents, open & overdue actions and injuries to verify — your safety pulse at a glance.' },
  { route: '/dashboard', sel: '[data-tour="dash-charts"]', title: 'Interactive charts', text: 'Severity, type, HSE category, location and a body-map heatmap. Click any segment to cross-filter the whole dashboard.' },
  { route: '/incidents', sel: '[data-tour="incidents-header"]', title: 'Incidents', text: 'Every report lives here. Hit “Report Incident” to start the guided 5-step investigation — it saves as you go.' },
  { route: '/incidents/illness', sel: '[data-tour="illness-header"]', title: 'Illness Reports', text: 'Log work-related illness — agent, exposure, PPE and corrective actions — with “Report Illness”.' },
  { route: '/incidents/injuries', sel: '[data-tour="injuries-header"]', title: 'Injury Reports', text: 'Each injured person gets a record to review and verify, with a detailed body map.' },
  { route: '/incidents/actions', sel: '[data-tour="actions-header"]', title: 'Action Tracker', text: 'All CAPA and illness actions, with owners and due dates. Overdue ones are flagged in red.' },
]
const TOUR_ADMIN = { route: '/users', sel: '[data-tour="users-header"]', title: 'Users (Admin)', text: 'Approve new sign-ups here and set each person’s role: Reporter, Investigator or Admin.' }
const TOUR_FINALE = { route: '/dashboard', place: 'home', title: 'I’m always here', text: 'Tap me anytime to ask things like “what’s overdue?” or “how do I report an incident?”. Enjoy Incident IRA!' }
...
export default function Assistant() {
  ...
  const tourKey = `ira:guide:tour:${uid}`
  useEffect(() => {
    ...
    try { sessionStorage.setItem(`ira:guide:greeted:${uid}`, '1') } catch { /* ignore */ }
  ...
  return (
    <div className="no-print">
      ...
      <motion.div
        ...
        onDragEnd={onDragEnd}
      >
        <button onClick={() => { if (tour) return; open ? setOpen(false) : openPanel() }} className="relative block" aria-label="Open Sam, the Safety Guide">
          {reduced ? (
