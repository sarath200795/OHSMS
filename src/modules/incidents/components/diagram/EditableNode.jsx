...
export default function EditableNode({ id, data, selected }) {
  ...
  // The toolbar is the KEYBOARD path, not a convenience. Renaming a node was
  // double-click and nothing else, so an investigator working a 5-Why or a
  // fishbone from the keyboard could select a node, delete it, flip a gate —
  // and not change a single label. React Flow makes nodes selectable with the
  // arrow keys, so putting Edit here is what completes the loop; double-click
  // stays as the mouse accelerator it always was.
