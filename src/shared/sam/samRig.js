...
export function buildSam(THREE) {
  const M = {
    ...
    reflect: new THREE.MeshStandardMaterial({ color: '#e8eef4', roughness: 0.25, metalness: 0.3 }),
    paper: new THREE.MeshStandardMaterial({ color: '#f8fafc', roughness: 0.9, metalness: 0 }),
  ...
  /** A rounded box: extruded rounded rect with a bevel. */
  ...
  for (const [side, sx] of [
    ...
    hip.add(part(`leg_${side}`, roundedBox(0.13, 0.4, 0.15, 0.05), M.navy, [0, -0.22, 0]))
    hip.add(
      part(`boot_${side}`, roundedBox(0.165, 0.12, 0.26, 0.05), M.navyDark, [0, -0.455, 0.038])
    )
  ...
  // ── Torso ──
  add(part('chest', roundedBox(0.36, 0.3, 0.25, 0.085), M.navy, [0, 0.8, 0]))
  add(part('waist', roundedBox(0.3, 0.22, 0.22, 0.07), M.navy, [0, 0.6, 0]))
  ...
  add(
    part('tape_front_L', roundedBox(0.038, 0.32, 0.012, 0.006), M.reflect, [-0.092, 0.755, 0.137])
  )
  add(part('tape_front_R', roundedBox(0.038, 0.32, 0.012, 0.006), M.reflect, [0.092, 0.755, 0.137]))
  add(part('tape_back_hi', roundedBox(0.3, 0.045, 0.012, 0.006), M.reflect, [0, 0.845, -0.137]))
  add(part('tape_back_lo', roundedBox(0.3, 0.045, 0.012, 0.006), M.reflect, [0, 0.675, -0.137]))
  ...
  const buildArm = (side, sx) => {
    ...
    shoulder.add(
      part(`shoulder_${side}`, new THREE.SphereGeometry(0.068, 20, 14), M.vest, [0, 0.005, 0])
    )
    shoulder.add(
      part(`sleeve_${side}`, new THREE.CapsuleGeometry(0.056, 0.15, 6, 18), M.navy, [0, -0.115, 0])
    )
    shoulder.add(
      part(
        `cuff_sleeve_${side}`,
        new THREE.CylinderGeometry(0.058, 0.055, 0.035, 18),
        M.navyDark,
        [0, -0.198, 0]
      )
    )
    ...
    elbow.add(
      part(
        `forearm_${side}`,
        new THREE.CapsuleGeometry(0.05, 0.13, 6, 18),
        M.skinShade,
        [0, -0.09, 0]
      )
    )
    elbow.add(
      part(`hand_${side}`, roundedBox(0.088, 0.1, 0.082, 0.032), M.skin, [0, -0.195, 0.008])
    )
    elbow.add(
      part(
        `thumb_${side}`,
        new THREE.CapsuleGeometry(0.019, 0.035, 4, 12),
        M.skin,
        [sx * -0.045, -0.175, 0.03],
        [0, 0, sx * 0.6]
      )
  ...
  clip.add(
    part('clipboard_sheet', roundedBox(0.18, 0.24, 0.008, 0.006), M.paper, [0, -0.014, 0.012])
  )
  clip.add(part('clipboard_clip', roundedBox(0.08, 0.03, 0.028, 0.008), M.steel, [0, 0.122, 0.014]))
  ;[0.062, 0.026, -0.01, -0.046].forEach((y, i) =>
    clip.add(
      part(`clipboard_rule_${i}`, roundedBox(0.12, 0.011, 0.005, 0.004), M.steel, [
        -0.012,
        y,
        0.018,
      ])
  ...
  clip.position.set(0.03, -0.235, 0.1)
  clip.rotation.set(0.3, 0.18, 0.46)
  armL.elbow.add(clip)
  ...
  sam.add(
    part('neck', new THREE.CylinderGeometry(0.055, 0.065, 0.075, 18), M.skinShade, [0, 0.975, 0])
  )
  sam.add(
    part('collar', new THREE.CylinderGeometry(0.085, 0.095, 0.04, 18), M.navyDark, [0, 0.955, 0])
  )
  ...
  H('hair_side_L', roundedBox(0.042, 0.17, 0.255, 0.026), M.hair, [-0.155, 1.2, -0.022])
  H('hair_side_R', roundedBox(0.042, 0.17, 0.255, 0.026), M.hair, [0.155, 1.2, -0.022])
  H('hair_back', roundedBox(0.315, 0.215, 0.05, 0.03), M.hair, [0, 1.185, -0.138])
  H(
    'hair_tuft',
    roundedBox(0.08, 0.065, 0.095, 0.032),
    M.hair,
    [0.062, 1.352, -0.015],
    [0, 0, -0.28]
  )
  ...
  for (const [side, sx] of [
    ...
    H(`eye_white_${side}`, new THREE.SphereGeometry(0.034, 18, 12), M.paper, [
      sx * 0.066,
      1.172,
      0.118,
    ])
    ...
    head.add(pupil)
    eyes.push(pupil)
    H(`eye_gleam_${side}`, new THREE.SphereGeometry(0.007, 10, 8), M.paper, [
      sx * 0.06,
      1.18,
      0.157,
    ])
    H(
      `brow_${side}`,
      roundedBox(0.068, 0.018, 0.022, 0.008),
      M.hair,
      [sx * 0.066, 1.232, 0.132],
      [0, 0, sx * 0.12]
    )
  }
  ...
  H(
    'mouth',
    new THREE.TorusGeometry(0.036, 0.008, 8, 20, Math.PI * 0.8),
    M.ink,
    [0, 1.098, 0.138],
    [0, 0, Math.PI + Math.PI * 0.1]
  )
