...
4. Project settings → Service accounts → **Generate new private key** → GitHub →
   ...
   **Then let it act as the functions runtime account.**  ⚠️ IAM

   Deploying a Gen-2 Cloud Function tells Cloud Run to *run as*
   `<project-number>-compute@developer.gserviceaccount.com`. Impersonating an
   account requires `iam.serviceaccounts.actAs`, which a freshly generated key
   does not have, so every function fails at deploy with:

   ```
   HTTP Error: 403, Caller is missing permission 'iam.serviceaccounts.actAs'
   on service account projects/-/serviceAccounts/<number>-compute@developer.gserviceaccount.com
   ```

   Grant it once, replacing the member with the `client_email` from the key
   generated above:

   ```bash
   gcloud iam service-accounts add-iam-policy-binding \
     <project-number>-compute@developer.gserviceaccount.com \
     --project=weehs-staging \
     --member="serviceAccount:<the deploy key's client_email>" \
     --role="roles/iam.serviceAccountUser"
   ```

   Console equivalent: IAM & Admin → Service Accounts → the `…-compute@developer`
   account → Permissions → Grant access → that member → **Service Account User**.

   **Finding the member is the fiddly part.** GitHub secrets are write-only, so
   the value cannot be read back out of the repo once set. Look it up in Cloud
   console → IAM & Admin → **Service Accounts** on the staging project instead;
   a key generated by `firebase init hosting:github` is named
   `github-action-<digits>@weehs-staging.iam.gserviceaccount.com`.

   **Why this hid for so long.** Only the functions step needs `actAs` — rules
   and hosting impersonate nothing and deploy fine without it. So the run goes
   red while the parts anyone would think to check are green, and the failure
   looks like a broken deploy rather than a missing grant. Staging sat red on
   every merge to main for long enough that a red staging run stopped meaning
   anything, which is the real cost: a genuine staging failure had nowhere left
   to be noticed.

   Production is not affected — `weehs-4eb28` was granted this at some point and
   its tagged deploys succeed. This is a per-project binding, so any NEW project
   that deploys functions needs it again.
