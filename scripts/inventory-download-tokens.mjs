...
//
// Dry by default. Nothing is revoked without --apply, and --apply still
// refuses without Admin credentials so a signed-in client cannot do this.
//
...
// every subcollection in POINTER_SUBCOLLECTIONS; a pointer in an unnamed
// collection is a gap this inventory will not see, which is why the dry run
// prints the walk list.
const NAMED = [
  'extinguishers',
  'trainingCourses',
  'documents',
...
console.log(`\nOrg ${orgId}`)
console.log(
  `Walked: org document, ${NAMED.join(', ')}, and ${POINTER_SUBCOLLECTIONS.map((s) => `${s.parent}/${s.sub}`).join(', ')}`
)
console.log(
...
if (!hasAdminCreds) {
  console.error(
    '\nRefusing --apply without Admin credentials.\n' +
      'The client SDK cannot strip firebaseStorageDownloadTokens; that write is a\n' +
      'bucket metadata change. Set GOOGLE_APPLICATION_CREDENTIALS to a service-account\n' +
      'JSON (or FIREBASE_SERVICE_ACCOUNT) and re-run. Review rows still will not be touched.\n'
  )
  process.exit(1)
}
